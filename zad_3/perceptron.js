class Perceptron {
    ...
    getPrzyklad(nrPrzykladu){
        let fftReal = [...numbers[nrPrzykladu]];
        let fftImag = new Array(36+1).join('0').split('').map(parseFloat);
        transform(fftReal, fftImag)
        let fftPrzyklad = new Array;
        for(let i = 0; i < 36; i++){
             fftPrzyklad.push(Math.pow(Math.pow(fftReal[i],2) + Math.pow(fftImag[i], 2),0.5));
        }
        return [...numbers[nrPrzykladu], ...fftPrzyklad];
    }
